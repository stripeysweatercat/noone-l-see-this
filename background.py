...
class MyClient(discord.Client):
    ...
    async def my_background_task(self):
        ...
        startPerigon = youtube.activities().list(
            part='contentDetails',
            channelId=f'{perigonID}'
        )
        startRocket = youtube.activities().list(
            part='contentDetails',
            channelId=f'{rocketID}'
        )
        startFibes = youtube.activities().list(
            part='contentDetails',
            channelId=f'{fibesID}'
        )
        startAustamate = youtube.activities().list(
            part='contentDetails',
            channelId=f'{austaID}'
        )
        startupPerigon = startPerigon.execute()
        startupRocket = startRocket.execute()
        startupFibes = startFibes.execute()
        startupAusta = startAustamate.execute()
        perigonPrevId = startupPerigon['items'][0]['contentDetails']['upload']['videoId']
        perigonCurrentId = 0
        rocketPrevId = startupRocket['items'][0]['contentDetails']['upload']['videoId']
        rocketCurrentId = 0
        fibesPrevId = startupFibes['items'][0]['contentDetails']['upload']['videoId']
        fibesCurrentId = 0
        austaPrevId = startupAusta['items'][0]['contentDetails']['upload']['videoId']
        austaCurrentId = 0
